...
def reduce_crop_type(year, start_day, end_day, sg_win, sg_poly):
    ...
    for filename in preprocessed_list:
        ...
        if crop_type in CROP_TYPES and year in filename and filename_feature_latter in filename:
...
def combiner(train_years, test_year, start_day, end_day, sg_win, sg_poly, note='REG'):
    ...
    train_label, test_label = label_maker(train_years, test_year, start_day, end_day, sg_win, sg_poly, note)
    ...
    np.savez(join(PRETRAIN_PATH, train_filename), features=batch_x_train, labels=batch_y_train)
    np.savez(join(PRETRAIN_PATH, test_filename), features=batch_x_test, labels=batch_y_test)


def batch_run():
    def batch_worker(process_item):
        train_years, test_year, start_day, end_day, sg_win, sg_poly = process_item
        combiner(train_years, test_year, start_day, end_day, sg_win, sg_poly, MODEL_NOTE)
...
if __name__ == '__main__':
    # batch_x, batch_y = single_combine('2017', '0501', '1001', '33', '2')
    # file_list = reduce_crop_type('2017', '0501', '1001', '33', '2')
    # print(file_list)
    # train_label, test_label = label_maker(['2014', '2015', '2016'], '2017', '0501', '1001', '33', '2', 'REG')
